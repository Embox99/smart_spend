...
const getProgressColor = (pct) => {
  if (pct >= 100) return { bar: "bg-red-500 dark:bg-red-500",   text: "text-red-600 dark:text-red-400"   };
  if (pct >= 80)  return { bar: "bg-amber-400 dark:bg-amber-400", text: "text-amber-600 dark:text-amber-400" };
  return           { bar: "bg-violet-500 dark:bg-violet-400",   text: "text-violet-600 dark:text-violet-400" };
};
  ...
  return (
    <div className="card group relative">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h6 className="font-medium text-gray-900 dark:text-white text-sm">{category}</h6>
          <p className="text-xs text-gray-400 dark:text-gray-500">Monthly limit</p>
        </div>
      ...
      <div className="flex justify-between text-xs text-gray-500 dark:text-gray-400">
        <span>
          Spent:{" "}
          <span className="font-medium text-gray-800 dark:text-gray-200">${spent.toLocaleString()}</span>
        </span>
        <span>
          Left:{" "}
          <span className={`font-medium ${remaining === 0 ? "text-red-500" : "text-gray-800 dark:text-gray-200"}`}>
